...
export default class ConfigInteractive extends BaseCommand {
    static description = 'Interactive configuration wizard for mastro';
    static examples = [
    ...
    async run() {
        ...
        try {
            ...
            while (configuring) {
                const selectedSection = await interactiveUI.selectFromList(configSections, section => section.label, 'Select configuration section');
                if (!selectedSection) {
                    configuring = false;
                    continue;
                }
                switch (selectedSection.key) {
                    ...
                    case 'save':
                        ...
                        break;
                    case 'exit':
                        const confirmExit = await interactiveUI.confirmAction('Exit without saving changes?', false);
                        if (confirmExit) {
                            configuring = false;
                        }
                        break;
    ...
    async configureAI(ui, currentAI) {
        this.log('\n🤖 AI Configuration', 'info');
        // Provider selection
        const providers = ['openai', 'anthropic'];
        const selectedProvider = await ui.selectFromList(providers, provider => provider === 'openai'
            ? 'OpenAI (ChatGPT)'
            : 'Anthropic (Claude)', 'Select AI provider', currentAI.provider) || currentAI.provider;
        // Model selection based on provider
        const models = selectedProvider === 'anthropic'
            ? ['claude-sonnet-4-0']
            : ['gpt-4o-mini', 'gpt-4.1-mini', 'gpt-5-mini'];
        const selectedModel = await ui.selectFromList(models, model => model, 'Select AI model', currentAI.model) || currentAI.model;
        // API Key input
        const currentKeyMasked = currentAI.apiKey ? '*'.repeat(20) : 'Not set';
        const updateKey = await ui.confirmAction(`Current API key: ${currentKeyMasked}. Update API key?`, false);
        let apiKey = currentAI.apiKey;
        if (updateKey) {
            const keyName = selectedProvider === 'anthropic' ? 'Anthropic' : 'OpenAI';
            const newKey = await ui.getTextInput(`Enter your ${keyName} API key`, undefined, true // hide input
            );
            if (newKey) {
                apiKey = newKey;
            }
        ...
        // Max tokens
        const maxTokensStr = await ui.getTextInput('Max tokens per request', currentAI.maxTokens.toString());
        const maxTokens = maxTokensStr ? parseInt(maxTokensStr, 10) : currentAI.maxTokens;
        // Temperature
        const temperatureStr = await ui.getTextInput('Temperature (0.0-2.0)', currentAI.temperature.toString());
    ...
    async configureGit(ui, currentGit) {
        this.log('\n📝 Git Configuration', 'info');
        const defaultBranch = await ui.getTextInput('Default branch name', currentGit.defaultBranch) || currentGit.defaultBranch;
        const includeUntracked = await ui.confirmAction('Include untracked files in analysis?', currentGit.includeUntracked);
        const maxDiffSizeStr = await ui.getTextInput('Maximum diff size (lines)', currentGit.maxDiffSize.toString());
    ...
    async configureTeam(ui, currentTeam) {
        this.log('\n👥 Team Configuration', 'info');
        // Commit style
        const commitStyles = ['conventional', 'custom'];
        const commitStyle = await ui.selectFromList(commitStyles, style => style === 'conventional'
            ? 'Conventional Commits (feat:, fix:, etc.)'
            : 'Custom team style', 'Select commit message style', currentTeam.commitStyle) || currentTeam.commitStyle;
        // Max commit message length
        const maxLengthStr = await ui.getTextInput('Maximum commit message length', currentTeam.maxLength.toString());
    ...
    async configureReviewPersona(ui, currentPersona) {
        this.log('\n📋 Review Persona Configuration', 'info');
        const name = await ui.getTextInput('Reviewer persona name', currentPersona.name) || currentPersona.name;
        const strictnessOptions = ['lenient', 'moderate', 'strict'];
        const strictness = await ui.selectFromList(strictnessOptions, s => s, 'Select review strictness', currentPersona.strictness) || currentPersona.strictness;
        ...
        for (const focus of focusOptions) {
            const shouldInclude = await ui.confirmAction(`Focus on ${focus}?`, currentFocusSet.has(focus));
            if (shouldInclude) {
    ...
    async configureCache(ui, currentCache) {
        this.log('\n💾 Cache Configuration', 'info');
        const enabled = await ui.confirmAction('Enable caching?', currentCache.enabled);
        const ttlStr = await ui.getTextInput('Cache TTL (seconds)', currentCache.ttl.toString());
        const ttl = ttlStr ? parseInt(ttlStr, 10) : currentCache.ttl;
        const maxSizeStr = await ui.getTextInput('Maximum cache entries', currentCache.maxSize.toString());
    ...
    async configureUI(ui, currentUI) {
        this.log('\n🎨 UI Configuration', 'info');
        const spinner = await ui.confirmAction('Show loading spinners?', currentUI.spinner);
        const colors = await ui.confirmAction('Use colored output?', currentUI.colors);
        const interactive = await ui.confirmAction('Enable interactive mode?', currentUI.interactive);
        return {
            spinner,
            colors,
            interactive
    ...
    async reviewConfiguration(ui, config) {
    ...
    async saveConfiguration(config, global) {
        try {
