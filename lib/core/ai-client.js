...
Generate commit messages that reflect your senior-level understanding of software engineering principles. Focus on clarity, precision, and long-term maintainability. Respond with valid json only. Return only json.`,
            ...
            const content = response.output_text ||
                (response.output?.[0]?.type === 'message' ? response.output[0].content?.[0]?.text : null);
            if (!content) {
...
Respond with JSON in this format:
{
  ...
  "migrationNotes": ["migration", "steps", "if", "needed"]
    ...
    buildPRPrompt(context) {
        const changesStr = context.changes.map(change => `- ${change.file} (${change.type}, +${change.insertions} -${change.deletions})`).join('\n');
...
Respond with JSON in this format:
{
  ...
  "blockers": [
    ...
    buildOptimizedCommitPrompt(context, allocation, complexityAnalysis) {
        const selectedChanges = allocation.selectedChanges;
        const patterns = context.repository.patterns;
        // Build change descriptions with importance context
        const changesStr = selectedChanges.map((change) => {
        ...
        const warningsStr = complexityAnalysis.warnings.length > 0
            ? `\n\nWarnings:\n${complexityAnalysis.warnings.map((w) => `- ${w.title}: ${w.message}`).join('\n')}`
            : '';
        return `Analyze these git changes and generate a commit message:
...
Respond with JSON in this format:
{
  ...
  "reasoning": "why this message was chosen"
    ...
    buildOptimizedExplanationPrompt(context, allocation, complexityAnalysis) {
        const selectedChanges = allocation.selectedChanges;
        ...
        const changesStr = selectedChanges.map((change) => {
            ...
            if (importance === 'critical' || importance === 'high') {
                ...
                const detailedHunks = change.hunks.slice(0, 3).map((hunk) => {
                    const contextLines = hunk.lines.slice(0, 15).map((l) => `    ${l.type}: ${l.content.substring(0, 120)}`).join('\n');
...
- Files analyzed: ${selectedChanges.length}/${context.changes.length} (prioritized by importance)`;
        ...
        const warningsContext = complexityAnalysis.warnings.length > 0
            ? `\n\nComplexity Warnings:\n${complexityAnalysis.warnings.map((w) => `- ${w.level.toUpperCase()}: ${w.title} - ${w.message}`).join('\n')}`
            : '';
  ...
  "reasoning": "explanation of how the refinement addresses the user's request"
    ...
    async validateCodeReview(review) {
        ...
        if (!Array.isArray(review.compliments)) {
            ...
            review.compliments = review.compliments.filter(compliment => compliment && typeof compliment === 'string' && compliment.trim().length > 0);
        }
        return review;
    }
