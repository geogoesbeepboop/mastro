...
export class AnthropicProvider {
    ...
    capabilities = {
        ...
        supportsJsonMode: false, // Anthropic doesn't have strict JSON mode like OpenAI
    ...
    constructor(config) {
        this.config = config;
        if (!config.apiKey) {
            throw new Error('Anthropic API key is required. Set ANTHROPIC_API_KEY environment variable or configure it in mastro.config.json');
        }
    ...
    async generateCommitMessage(context) {
        ...
        try {
            const response = await this.client.messages.create({
                ...
                system: `You are a Principal Software Engineer with 15+ years of experience leading engineering teams at top-tier technology companies. Your responsibility is to ensure the highest standards of code quality, maintainability, and engineering best practices across the entire codebase.
    ...
    async refineCommitMessage(originalMessage, refinementInstruction, context) {
        ...
        try {
            const response = await this.client.messages.create({
                ...
                system: `You are a Principal Software Engineer with 15+ years of experience. Your task is to refine commit messages based on specific instructions while maintaining technical accuracy and team conventions.
    ...
    async explainChanges(context) {
        ...
        try {
            const response = await this.client.messages.create({
                ...
                system: `You are a Principal Software Engineer and Technical Lead responsible for code quality and system architecture. Your deep expertise spans multiple domains: system design, performance optimization, security, scalability, and team leadership.
    ...
    async createPRDescription(context) {
        ...
        try {
            const response = await this.client.messages.create({
                ...
                system: `You are a Principal Software Engineer and Technical Lead responsible for maintaining the highest standards of code delivery and team collaboration. You have extensive experience with code reviews, release management, and cross-functional team coordination.
    ...
    async reviewCode(context, persona) {
        ...
        try {
            const response = await this.client.messages.create({
                ...
                system: `You are a Principal Software Engineer acting as a ${persona.name} conducting a ${persona.strictness} code review. You have the authority and responsibility to ensure this code meets production standards and company engineering guidelines.
...
Conduct this review with the rigor and insight expected from a principal-level engineer. Be thorough, authoritative, and focused on system reliability and team success. Respond with valid JSON only.`
    ...
    async generateDocumentation(type, context, config) {
        ...
        try {
            const response = await this.client.messages.create({
                model: this.config.model,
                max_tokens: this.config.maxTokens * 2, // Use more tokens for documentation
                ...
                system: `You are a Principal Technical Writer and Software Architect with extensive experience creating comprehensive technical documentation. Your expertise spans API documentation, system architecture, user guides, and technical communication.
...
Return only the markdown documentation content, no JSON wrapper.`
    ...
    buildCommitPrompt(context) {
        ...
        const isDocs = (file) => ((/^docs\//i).test(file) || (/\.md$/i).test(file));
        ...
        const changesStr = orderedChanges.map(change => `File: ${change.file} (${change.type}, +${change.insertions} -${change.deletions})`).join('\n');
...
Respond with JSON in this format:
{
  ...
  "dependencies": ["dependencies or prerequisites identified from changes"]
    ...
    buildReviewPrompt(context, persona) {
        const changesStr = context.changes.map(change => `File: ${change.file}\n${change.hunks.map(hunk => hunk.lines.map(l => l.content).join('\n')).join('\n')}`).join('\n\n');
...
Respond with JSON in this format:
{
  ...
  "blockers": [
    ...
    buildRefinementPrompt(originalMessage, refinementInstruction, context) {
        const patterns = context.repository.patterns;
...
Files involved:
${context.changes.slice(0, 5).map(change => `- ${change.file} (${change.type}, +${change.insertions} -${change.deletions})`).join('\n')}
...
Respond with JSON in this format:
{
  ...
  "reasoning": "explanation of how the refinement addresses the user's request"
    ...
    buildDocumentationPrompt(type, context, config) {
...
${context.recentCommits ? `Recent commits: ${context.recentCommits.slice(0, 3).map((c) => c.subject).join(', ')}` : ''}`;
        switch (type) {
            case 'api':
...
Please create detailed, practical documentation that helps users understand and work with this project effectively.`;
    ...
    buildApiDocPrompt(baseContext, projectStructure, codeAnalysis) {
        const apiFiles = projectStructure.files.filter((f) => f.apis && f.apis.length > 0);
        const functionFiles = projectStructure.files.filter((f) => f.functions && f.functions.length > 0);
        let apiDetails = '';
            ...
            for (const file of apiFiles.slice(0, 10)) {
                ...
                for (const api of file.apis.slice(0, 5)) {
                    ...
                    if (api.parameters && api.parameters.length > 0) {
                        apiDetails += `    Parameters: ${api.parameters.map((p) => `${p.name}(${p.dataType})`).join(', ')}\n`;
                    }
                }
            }
        }
...
## Mastro CLI Commands to Document
Include comprehensive documentation for these key commands:
- **mastro flow**: Complete workflow orchestration with boundary processing
- **mastro review**: AI-powered code review with enhanced compliment validation
- **mastro split**: Intelligent commit boundary analysis and staging
- **mastro commit**: Enhanced commit message generation with AI
- **mastro pr**: Pull request creation and management
- **mastro docs**: Multi-format documentation generation
- **mastro analytics**: Workflow analytics and session tracking

Create documentation that includes:
1. **CLI Command Reference**: Complete command documentation with examples
2. **Workflow Integration**: How commands work together in mastro flow
3. **API Architecture**: Internal API structure and design principles
4. **Configuration**: Settings and customization options
5. **Authentication**: AI provider setup and API keys
6. **Error Handling**: Common issues and troubleshooting
7. **Integration Examples**: Real-world usage scenarios
8. **Advanced Features**: Checkpoint management, boundary analysis
9. **Extension Points**: How to extend and customize the system

Focus on practical examples showing how to use mastro flow for complete development workflows. Include troubleshooting for common issues like the recent compliment validation improvements.`;
    }
    buildArchitectureDocPrompt(baseContext, projectStructure, codeAnalysis) {
        let patternInfo = '';
        ...
        if (codeAnalysis.dependencies && codeAnalysis.dependencies.length > 0) {
            depInfo = '\n\nKey Dependencies:\n';
            const prodDeps = codeAnalysis.dependencies.filter((d) => d.type === 'production').slice(0, 10);
...
Directory Structure:
${projectStructure.directories.slice(0, 15).map((d) => `${d.path}${d.description ? ' - ' + d.description : ''}`).join('\n')}

## Mastro Flow Architecture
The system implements a sophisticated workflow orchestration architecture:

### Core Components
- **WorkflowContextManager**: Manages workflow state and checkpoints
- **CommitBoundaryAnalyzer**: Intelligent code boundary detection
- **AIClient**: Enhanced AI integration with validation improvements
- **DocumentationEngine**: Multi-format documentation generation
- **ReviewEngine**: Code review with compliment validation fixes

### Workflow Orchestration
- **Flow Command**: Complete workflow coordination
- **Boundary Processing**: Smart commit grouping and staging
- **Checkpoint System**: Workflow recovery and continuation
- **Multi-provider AI**: Support for Anthropic and OpenAI

Create documentation that includes:
1. **System Architecture Overview**: Mastro flow workflow orchestration
2. **Component Architecture**: Core modules and their interactions
3. **Workflow Engine**: How mastro flow orchestrates the complete pipeline
4. **AI Integration Architecture**: Enhanced validation and prompt systems
5. **Documentation System**: Multi-format generation with Mermaid support
6. **Error Recovery**: Checkpoint system and workflow continuation
7. **Extensibility**: Plugin architecture and customization points
8. **Performance Design**: Async processing and optimization
9. **Security Considerations**: AI API handling and data protection
10. **Recent Improvements**: Compliment validation fixes and enhancements
11. **Mermaid Diagrams**: Visual representation of workflow and component relationships

Focus on the innovative workflow orchestration system and how it enables automated development workflows. Include architectural decisions that support the recent enhancements.`;
    }
    buildUserGuideDocPrompt(baseContext, projectStructure, codeAnalysis) {
        let userFlowInfo = '';
...
Write for users who want to accomplish tasks efficiently, not necessarily developers. Focus on practical outcomes and clear instructions.`;
    ...
    buildReadmeDocPrompt(baseContext, projectStructure, codeAnalysis) {
        return `Generate a comprehensive README.md for this project:

${baseContext}

Key directories: ${projectStructure.directories.slice(0, 8).map((d) => d.path.split('/').pop()).join(', ')}
...
Make it compelling for both users and contributors. Focus on getting people excited about the project and helping them get started quickly.`;
    ...
    buildWorkflowDocPrompt(baseContext, projectStructure, codeAnalysis) {
...
Focus on practical examples and real-world scenarios. Include step-by-step guides for the complete mastro flow experience.`;
    ...
    buildIntegrationDocPrompt(baseContext, projectStructure, codeAnalysis) {
...
Focus on practical integration examples and real-world deployment scenarios.`;
    ...
    hasSpecificTechnicalTerms(title) {
    ...
    async validateCodeReview(review) {
        ...
        if (!Array.isArray(review.blockers)) {
            ...
            const exists = async (p) => !!p && await fs.access(p).then(() => true).catch(() => false);
        ...
        if (!Array.isArray(review.compliments)) {
            ...
            review.compliments = review.compliments
                ...
                .filter(compliment => compliment && typeof compliment === 'string' && compliment.trim().length > 0);
        }
        return review;
    }
    ...
    async analyzeSpecificIssue(file, line, type) {
        try {
...
Be specific and actionable. Focus on the exact problem in the code.`;
            const response = await this.client.messages.create({
                ...
                system: "You are a code analysis expert. Provide a detailed analysis of the specific code issue. Be precise, technical, and actionable. Respond in valid JSON format with 'message' and 'suggestion' fields only."
            });
            const content = response.content[0];
            if (content.type === 'text') {
                const parsed = JSON.parse(content.text);
