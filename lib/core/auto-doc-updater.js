import { ChangeDetector } from '../analyzers/change-detector.js';
...
export class AutoDocumentationUpdater {
    ...
    aiClient;
    config;
    constructor(config, aiClient, outputDir = './docs') {
        this.config = config;
    ...
    async analyzeAndUpdateDocumentation(context, updateConfig) {
        const summary = {
            changesDetected: [],
            documentsUpdated: [],
            documentsCreated: [],
        ...
        try {
            ...
            const significantChanges = changeAnalyses.filter(change => change.confidence >= updateConfig.threshold);
            if (significantChanges.length === 0) {
                summary.suggestions.push(`No changes met the confidence threshold of ${updateConfig.threshold}. ` +
            ...
            for (const docType of docTypesToUpdate) {
                try {
                    const updateResult = await this.updateOrCreateDocument(docType, docContext, significantChanges, updateConfig);
    ...
    determineDocumentUpdates(changes, config) {
    ...
    async buildDocumentationContext(context) {
        // Reuse the same context building logic from the documentation generator
        return {
            repository: context.repository,
            projectStructure: {
                files: [], // Would be populated by actual project analysis
                directories: [],
                entryPoints: [],
                configFiles: [],
                testFiles: [],
            ...
            codeAnalysis: {
                complexity: { overall: 'medium', metrics: { totalLines: 0, apiEndpoints: 0 } },
                patterns: [],
                dependencies: [],
                userFlows: []
            },
    ...
    async updateOrCreateDocument(docType, docContext, changes, config) {
        const docConfig = {
    ...
    async updateExistingDocument(docType, docContext, changes, docConfig) {
        ...
        const updatedDoc = await this.docEngine.generateDocumentation(docType, enhancedDocContext, {
            ...docConfig,
            contextAware: true,
    ...
    async intelligentMerge(existing, updated, changes) {
        ...
        const synthesizedContent = await this.synthesizeDocumentationContent(existingStructure, updatedStructure, changes);
    ...
    async synthesizeDocumentationContent(existing, updated, changes) {
...
`;
        try {
            const synthesizedContent = await this.aiClient.performCustomAnalysis(contextPrompt, 'Synthesize the best possible documentation from existing and updated content.', 2000, 0.3);
    ...
    fallbackIntelligentMerge(existing, updated, changes) {
    ...
    generateSuggestions(changes, summary) {
        const suggestions = [];
    ...
    async previewUpdates(context, updateConfig) {
...
//# sourceMappingURL=auto-doc-updater.js.map