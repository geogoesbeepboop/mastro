...
exports.LiveQualityEngine = void 0;
...
class LiveQualityEngine {
    ...
    async analyzeComplexity(document, content) {
        const issues = [];
        const suggestions = [];
    ...
    async analyzeMaintainability(document, content) {
        const issues = [];
        const suggestions = [];
        let score = 100;
        const lines = content.split('\n');
    ...
    async analyzePerformance(document, content) {
        const issues = [];
        const suggestions = [];
        let score = 100;
        const lines = content.split('\n');
    ...
    async analyzeSecurity(document, content) {
        const issues = [];
        const suggestions = [];
        let score = 100;
        const lines = content.split('\n');
    ...
    async analyzeTestCoverage(document, filePath) {
        const issues = [];
        const suggestions = [];
    ...
    async analyzeDocumentation(document, content) {
        const issues = [];
        const suggestions = [];
        let score = 100;
        const lines = content.split('\n');
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (functionMatch) {
                ...
                const isDocumented = i > 0 && (lines[i - 1].trim().includes('*/') ||
    ...
    async updateDiagnostics(document, metrics) {
        const diagnostics = [];
        for (const metric of metrics) {
            for (const issue of metric.issues) {
                const diagnostic = new vscode.Diagnostic(new vscode.Range(issue.line, issue.column, issue.endLine, issue.endColumn), issue.message, this.severityToDiagnosticSeverity(issue.severity));
    ...
    updateConfiguration() {
        ...
        this.config = {
            complexityThreshold: config.get('complexityThreshold', 8),
            maintainabilityThreshold: config.get('maintainabilityThreshold', 70),
            performanceThreshold: config.get('performanceThreshold', 80),
            securityThreshold: config.get('securityThreshold', 90),
            testCoverageThreshold: config.get('testCoverageThreshold', 70),
            documentationThreshold: config.get('documentationThreshold', 60)
        };
    }
    ...
    generateProjectRecommendations(overallScore, topIssues, hotspots) {
...
exports.LiveQualityEngine = LiveQualityEngine;
//# sourceMappingURL=liveQualityEngine.js.map