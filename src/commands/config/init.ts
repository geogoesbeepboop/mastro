...
export default class ConfigInit extends BaseCommand {
  static override description = 'Initialize mastro configuration';
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      if (hasExisting && !flags.force) {
        const overwrite = await interactiveUI.confirmAction(
          `Configuration already exists at ${this.configManager.getConfigPath(flags.global)}. Overwrite?`,
          false
        );
      ...
      if (this.mastroConfig.ui.interactive) {
        const styleChoice = await interactiveUI.selectFromList(
          commitStyles,
          style => style === 'conventional' ? 'Conventional Commits (feat: fix: etc.)' : 'Custom team style',
