...
import type {HookConfig} from '../core/hook-generator.js';

export default class Hooks extends BaseCommand {
  static override description = 'Manage intelligent pre-commit hooks for AI-powered code validation';
  ...
  private async installHooks(flags: any): Promise<void> {
    ...
    console.log('\n' + this.renderer.renderSection('Configuration Summary', [
      `Strictness: ${this.renderer.renderPriority(hookConfig.strictness as any)}`,
    ...
    if (!flags.force) {
      const proceed = await this.interactiveUI.confirmAction(
        'Install hooks with this configuration?',
        true
      );
  ...
  private async uninstallHooks(flags: any): Promise<void> {
    console.log('\n' + this.renderer.renderTitle('🗑️ Uninstalling Mastro Hooks'));

    // Confirm uninstallation unless forced
    if (!flags.force) {
      const proceed = await this.interactiveUI.confirmAction(
        'Are you sure you want to uninstall mastro hooks? This will remove AI-powered validation.',
        false // Default to no
      );
  ...
  private async configureHooks(flags: any): Promise<void> {
  ...
  private async interactiveConfiguration(): Promise<HookConfig> {
    ...
    const strictness = await this.interactiveUI.selectIndex(
      'Select hook strictness level:',
      [
    ...
    const personaIndex = await this.interactiveUI.selectIndex(
      'Select review focus:',
      [
    ...
    return {
      type: 'pre-commit',
      enabled: true,
      strictness: strictnessOptions[strictness] as any,
      criticalThreshold,
      highThreshold,
      persona: {
        name: personaOptions[personaIndex],
        focus: [personaOptions[personaIndex] as any],
        strictness: strictnessOptions[strictness] as any,
        customRules: []
      },
      customRules: [],
  ...
  private buildHookConfig(flags: any): HookConfig {
    return {
      type: 'pre-commit',
      enabled: true,
