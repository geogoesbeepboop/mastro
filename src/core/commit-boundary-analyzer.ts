...
export interface CommitBoundary {
  ...
  theme: string; // What this commit is about (e.g., "authentication", "UI styling")
}
...
export class CommitBoundaryAnalyzer {
   ...
   */
  private async analyzeFileRelationships(changes: GitChange[]): Promise<FileRelationship[]> {
   ...
   */
  private async groupByImpact(changes: GitChange[]): Promise<Map<string, GitChange[]>> {
   ...
   */
  private async buildDependencyGraph(changes: GitChange[]): Promise<Map<string, string[]>> {
   ...
   */
  private async detectBoundariesML(
    relationships: FileRelationship[],
    impactGroups: Map<string, GitChange[]>,
    dependencyGraph: Map<string, string[]>
  ): Promise<CommitBoundary[]> {
    const boundaries: CommitBoundary[] = [];
   ...
   */
  private optimizeBoundaries(boundaries: CommitBoundary[]): CommitBoundary[] {
    const optimized: CommitBoundary[] = [];
  ...
  private async checkImportRelationship(file1: GitChange, file2: GitChange): Promise<{strength: number}> {
  ...
  private checkTestRelationship(file1: GitChange, file2: GitChange): {strength: number} {
  ...
  private async checkSimilarChanges(file1: GitChange, file2: GitChange): Promise<{strength: number}> {
  ...
  private checkConfigRelationship(file1: GitChange, file2: GitChange): {strength: number} {
  ...
  private categorizeImpactType(change: GitChange, analysis: any): string[] {
    const types: string[] = [];
  ...
  private determinePriority(impactType: string, files: GitChange[]): 'high' | 'medium' | 'low' {
    const highPriorityTypes = ['business_logic', 'api', 'database'];
    const mediumPriorityTypes = ['ui', 'configuration'];
    
    if (highPriorityTypes.includes(impactType)) return 'high';
    if (mediumPriorityTypes.includes(impactType)) return 'medium';
    return 'low';
  }

  private calculateComplexity(files: GitChange[]): number {
    let complexity = 0;
  ...
  private async findFileDependencies(change: GitChange, allChanges: GitChange[]): Promise<string[]> {
    const dependencies: string[] = [];
    const content = change.hunks.map(h => h.lines.map(l => l.content).join('\n')).join('\n');
    
    for (const otherChange of allChanges) {
      if (otherChange.file === change.file) continue;
  ...
  private findBoundaryDependencies(files: GitChange[], dependencyGraph: Map<string, string[]>): string[] {
  ...
  private extractFunctionNames(change: GitChange): string[] {
  ...
  private splitLargeBoundary(boundary: CommitBoundary): CommitBoundary[] {
    const chunks: GitChange[][] = [];
  ...
  private tryMergeSmallBoundary(boundary: CommitBoundary, existingBoundaries: CommitBoundary[]): boolean {
  ...
  private async generateCommitMessage(boundary: CommitBoundary): Promise<{title: string; type: string; body?: string}> {
  ...
  private inferCommitType(boundary: CommitBoundary): string {
    if (boundary.theme.includes('test')) return 'test';
    if (boundary.theme.includes('doc')) return 'docs';
    if (boundary.theme.includes('config')) return 'chore';
    if (boundary.theme.includes('ui') || boundary.theme.includes('style')) return 'feat';
    if (boundary.files.some(f => f.file.includes('fix') || f.file.includes('bug'))) return 'fix';
    return 'feat';
  }

  private inferScope(boundary: CommitBoundary): string | null {
  ...
  private generateDescription(boundary: CommitBoundary): string {
    if (boundary.theme === 'authentication') return 'implement authentication system';
    if (boundary.theme === 'user interface') return 'update UI components';
    if (boundary.theme === 'api development') return 'add API endpoints';
    if (boundary.theme === 'testing') return 'add test coverage';
    if (boundary.theme === 'configuration') return 'update configuration';
  ...
  private generateRationale(boundary: CommitBoundary): string {
    return `This commit groups ${boundary.files.length} files related to ${boundary.theme}. ${boundary.reasoning}`;
  }

  private assessCommitRisk(boundary: CommitBoundary): 'low' | 'medium' | 'high' {
    if (boundary.estimatedComplexity > 500) return 'high';
    if (boundary.estimatedComplexity > 200) return 'medium';
    return 'low';
  }

  private estimateCommitTime(boundary: CommitBoundary): string {
    const baseTime = Math.max(2, Math.ceil(boundary.files.length / 2));
    return `${baseTime} minutes`;
  }

  private identifyPotentialIssues(boundaries: CommitBoundary[]): string[] {
  ...
  private determineOptimalStrategy(boundaries: CommitBoundary[]): 'progressive' | 'parallel' | 'sequential' {
    ...
    if (hasHighRisk) return 'progressive';
    return 'parallel';
  }

  private assessOverallRisk(boundaries: CommitBoundary[]): 'low' | 'medium' | 'high' {
