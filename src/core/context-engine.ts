...
export class ContextEngine {
  ...
  async analyzeSemanticChanges(context: CommitContext): Promise<SemanticChange[]> {
  ...
  async analyzeImpact(context: CommitContext): Promise<ImpactAnalysis> {
    ...
    try {
      ...
      switch (ext) {
        ...
        case '.py': {
          ...
          if (importMatches) {
            dependencies.push(...importMatches.map(match => 
              match.replace(/^from\s+(\S+)\s+import.*/, '$1')
  ...
  private async analyzeFileChange(change: GitChange, context: CommitContext): Promise<SemanticChange | null> {
  ...
  private inferChangeType(change: GitChange, fileName: string, fileExt: string): ChangeType | null {
    // Test files
    ...
    if (change.type === 'added') return 'feat';
    if (change.type === 'modified') return 'fix';

    return null;
  }

  private detectNewFeatures(change: GitChange): boolean {
  ...
  private detectBugFixes(change: GitChange): boolean {
    for (const hunk of change.hunks) {
  ...
  private detectRefactoring(change: GitChange): boolean {
  ...
  private async generateDescription(change: GitChange, changeType: ChangeType): Promise<string> {
  ...
  private calculateConfidence(change: GitChange, changeType: ChangeType, fileName: string): number {
  ...
  private detectSemanticPatterns(change: GitChange, changeType: ChangeType): boolean {
    const allContent = change.hunks.flatMap(hunk => 
      hunk.lines.map(line => line.content.toLowerCase())
    ).join(' ');

    const patterns: Record<ChangeType, RegExp[]> = {
    ...
    return relevantPatterns.some((pattern: RegExp) => pattern.test(allContent));
  }

  private explainReasoning(change: GitChange, changeType: ChangeType, fileName: string): string {
  ...
  private consolidateChanges(changes: SemanticChange[]): SemanticChange[] {
    const consolidated = new Map<string, SemanticChange>();
  ...
  private async identifyAffectedComponents(changes: GitChange[]): Promise<string[]> {
  ...
  private async identifyPotentialIssues(changes: GitChange[]): Promise<string[]> {
    const issues: string[] = [];
  ...
  private async generateTestingRecommendations(changes: GitChange[]): Promise<string[]> {
