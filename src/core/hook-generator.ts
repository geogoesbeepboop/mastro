...
export class HookGenerator {
   ...
   */
  async generatePreCommitHook(hookConfig: HookConfig): Promise<void> {
   ...
   */
  async generatePrePushHook(hookConfig: HookConfig): Promise<void> {
   ...
   */
  async generateCommitMsgHook(hookConfig: HookConfig): Promise<void> {
   ...
   */
  async installHookSuite(hookConfig: HookConfig): Promise<void> {
    ...
    console.log('🔧 Installing intelligent pre-commit hook suite...');
    ...
    console.log('🎉 Hook suite installation complete!');
    console.log('');
  ...
  async uninstallHooks(): Promise<void> {
    ...
    for (const hook of hooks) {
      ...
      try {
        ...
        if (content.includes('# Generated by mastro')) {
          console.log(`🗑️ Removing mastro ${hook} hook`);
          ...
          try {
            ...
            console.log(`✅ Restored original ${hook} hook`);
          } catch {
    ...
    try {
      ...
      console.log('🗑️ Removed hook configuration');
    } catch {
      // Config doesn't exist
    }

    console.log('✅ Mastro hooks uninstalled successfully');
  }

  /**
   * Validate current hook configuration and suggest improvements
   */
  async validateHookSetup(): Promise<HookValidationResult> {
  ...
  private buildPreCommitHookContent(config: HookConfig): string {
    return `#!/bin/bash
...
if [ "$REVIEW_OUTPUT" = "ERROR" ]; then
    echo "⚠️ mastro: Analysis failed, proceeding with commit"
...
echo "📊 Found: \$CRITICAL_COUNT critical, \$HIGH_COUNT high priority issues"

# Block commit if critical issues exceed threshold
if [ "\$CRITICAL_COUNT" -gt ${config.criticalThreshold} ]; then
    echo ""
    echo "❌ Commit blocked: \$CRITICAL_COUNT critical issues found (threshold: ${config.criticalThreshold})"
    echo ""
...
if [ "\$HIGH_COUNT" -gt ${config.highThreshold} ]; then
    echo ""
    echo "⚠️  \$HIGH_COUNT high priority issues detected:"
...
' || {
    echo "⏰ mastro: Analysis timed out after ${config.timeoutSeconds}s"
    echo "💡 Consider increasing timeout or reducing changeset size"
...
exit 0`;
  ...
  private buildPrePushHookContent(config: HookConfig): string {
    return `#!/bin/bash
# Generated by mastro - Intelligent Pre-push Hook
# Generated: ${new Date().toISOString()}

echo "🚀 mastro: Validating branch before push..."
...
if [ -n "\$UNCOMMITTED" ]; then
    ...
    echo "\$UNCOMMITTED"
    echo ""
    echo "💡 Consider committing or stashing changes before push"
...
echo "🔍 Analyzing recent commits for quality issues..."
...
if [ "\$COMMIT_COUNT" -gt 10 ]; then
    echo "⚠️ \$COMMIT_COUNT commits being pushed - consider smaller, more frequent pushes"
fi

echo "✅ mastro: Pre-push validation completed"
exit 0`;
  }

  private buildCommitMsgHookContent(config: HookConfig): string {
    return `#!/bin/bash
...
echo "📝 mastro: Validating commit message..."

# Basic commit message validation
if [ \${#COMMIT_MSG} -lt 10 ]; then
    echo "❌ Commit message too short (minimum 10 characters)"
    echo "Current: '\$COMMIT_MSG'"
    echo "💡 Use \\"mastro commit\\" for AI-generated messages"
    exit 1
fi
...
if [ \${#COMMIT_MSG} -gt 72 ]; then
    ...
    if [ \${#FIRST_LINE} -gt 72 ]; then
        echo "⚠️ First line too long (\${#FIRST_LINE} chars, recommended: ≤72)"
        echo "💡 Consider shortening or use \\"mastro commit\\" for optimized messages"
    fi
fi
...
if echo "\$COMMIT_MSG" | grep -qE "^(feat|fix|docs|style|refactor|test|chore)(\(.+\))?: .+"; then
    ...
    echo "💡 Use \\"mastro commit\\" for consistent formatting"
fi

echo "✅ mastro: Commit message validation passed"
exit 0`;
  }
