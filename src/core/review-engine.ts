...
export class ReviewEngine {
  ...
  private complexityAnalyzer: ChangeComplexityAnalyzer;
  ...
  private generateWorkflowSuggestions(session: DevelopmentSession, analysis: any): WorkflowSuggestion[] {
    ...
    // Testing suggestions
    const changedFiles = [...session.workingChanges, ...session.stagedChanges];
    const hasLogicChanges = changedFiles.some(c => 
      c.hunks.some(h => 
        h.lines.some(l => 
          l.type === 'added' && 
    ...
    const hasPublicAPIChanges = changedFiles.some(c =>
      c.hunks.some(h =>
        h.lines.some(l =>
          l.type === 'added' &&
  ...
  private generateLearningPoints(session: DevelopmentSession, review: CodeReview, analysis: any): string[] {
    const points: string[] = [];
  ...
  private createEmptySessionReview(session: DevelopmentSession, persona: ReviewPersona): SessionReview {
    return {
      sessionId: session.id,
      scope: 'session',
      overall: {
  ...
  private mapSuggestionTypeToActionable(type: string): ActionableItem['type'] {
    switch (type) {
      case 'bug': return 'fix';
  ...
  private mapSeverityToPriority(severity: string): ActionableItem['priority'] {
    switch (severity) {
  ...
  private generateActionableTitle(suggestion: ReviewSuggestion): string {
    return `${suggestion.type.charAt(0).toUpperCase()}${suggestion.type.slice(1)}: ${suggestion.message.split('.')[0]}`;
  }

  private estimateEffort(suggestion: ReviewSuggestion): ActionableItem['estimatedEffort'] {
  ...
  private priorityWeight(priority: ActionableItem['priority']): number {
    switch (priority) {
