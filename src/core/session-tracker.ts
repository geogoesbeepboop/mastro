...
export class SessionTracker {
  ...
  async initializeSession(): Promise<DevelopmentSession> {
  ...
  async getCurrentSession(): Promise<DevelopmentSession> {
  ...
  async updateSessionState(): Promise<void> {
    ...
    // Update duration
    this.currentSession.cumulativeStats.duration = Math.floor(
      (Date.now() - this.currentSession.startTime.getTime()) / (1000 * 60)
    );
  }

  async getWorkingDirectoryChanges(): Promise<GitChange[]> {
  ...
  private async analyzeFileChange(file: string, type: 'added' | 'modified' | 'deleted'): Promise<GitChange | null> {
    try {
  ...
  private assessSessionRisk(): SessionRisk {
    if (!this.currentSession) {
      throw new Error('No active session');
    }
    ...
    const allChanges = [...this.currentSession.workingChanges, ...this.currentSession.stagedChanges];
    const hasSystemFiles = allChanges.some(c => 
      c.file.includes('package.json') || 
    ...
    const hasSecurityFiles = allChanges.some(c => 
      c.file.includes('.env') ||
  ...
  private detectSessionPatterns(): SessionPattern[] {
    if (!this.currentSession) {
      throw new Error('No active session');
    }
    ...
    if (this.currentSession.baseBranch !== 'main' && this.currentSession.baseBranch !== 'master') {
      ...
      const isFeatureBranch = featureKeywords.some(keyword => 
        this.currentSession.baseBranch.toLowerCase().includes(keyword)
      );
    ...
    // Refactoring pattern
    const refactoringIndicators = allChanges.filter(c => 
      c.hunks.some(h => h.lines.some(l => 
        (l.type === 'removed' || l.type === 'added') &&
        (l.content.includes('function') || l.content.includes('class') || l.content.includes('import'))
      ))
    ).length;
    ...
    const hasErrorHandling = allChanges.some(c => 
      c.hunks.some(h => h.lines.some(l => 
        l.type === 'added' && 
  ...
  async hasSessionChanges(): Promise<boolean> {
    if (!this.currentSession) {
      return false;
    }
