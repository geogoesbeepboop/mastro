...
export class WorkflowAnalyzer {
  ...
  async analyzeWorkflowPatterns(session: DevelopmentSession): Promise<SessionPattern[]> {
    ...
    if (tddPattern) patterns.push(tddPattern);

    // Feature Flag Pattern
    const featureFlagPattern = this.detectFeatureFlagPattern(allChanges);
    if (featureFlagPattern) patterns.push(featureFlagPattern);

    // Microcommit Pattern
    const microcommitPattern = this.detectMicrocommitPattern(session);
    if (microcommitPattern) patterns.push(microcommitPattern);
    ...
    if (docsPattern) patterns.push(docsPattern);

    // Hotfix Pattern
    const hotfixPattern = this.detectHotfixPattern(session);
    if (hotfixPattern) patterns.push(hotfixPattern);
  ...
  async detectPRType(session: DevelopmentSession): Promise<PRTemplate['type']> {
    ...
    // Content analysis
    const hasNewFeatures = allChanges.some(c => 
    ...
    const hasOnlyTests = allChanges.every(c => 
      c.file.includes('test') || c.file.includes('spec')
    );

    const hasOnlyDocs = allChanges.every(c => 
      c.file.includes('.md') || c.file.includes('doc') || c.file.includes('README')
    );

    const hasBugFixes = allChanges.some(c =>
      c.hunks.some(h => h.lines.some(l =>
    ...
    if (hasOnlyTests) return 'refactor';
    if (hasBugFixes && !hasNewFeatures) return 'bugfix';
    if (hasNewFeatures) return 'feature';

    return 'feature'; // Default fallback
  }

  async generateWorkflowSuggestions(session: DevelopmentSession): Promise<WorkflowSuggestion[]> {
    ...
    const hasPublicAPIs = allChanges.some(c =>
      c.hunks.some(h => h.lines.some(l =>
        l.type === 'added' && 
  ...
  async analyzeTeamWorkflow(recentSessions: DevelopmentSession[]): Promise<TeamWorkflowInsights> {
  ...
  async optimizeCommitStrategy(session: DevelopmentSession): Promise<string[]> {
    const allChanges = [...session.workingChanges, ...session.stagedChanges];
    ...
    const hasFeatureFlags = allChanges.some(c => 
      c.hunks.some(h => h.lines.some(l => 
        l.content.includes('feature') && l.content.includes('flag')
      ))
    );
  ...
  private detectTDDPattern(changes: GitChange[], session: DevelopmentSession): SessionPattern | null {
  ...
  private detectFeatureFlagPattern(changes: GitChange[]): SessionPattern | null {
    const hasFeatureFlags = changes.some(c =>
      c.hunks.some(h => h.lines.some(l =>
        l.content.includes('featureFlag') ||
  ...
  private detectMicrocommitPattern(session: DevelopmentSession): SessionPattern | null {
    const stats = session.cumulativeStats;
  ...
  private detectRefactoringSprintPattern(changes: GitChange[], stats: any): SessionPattern | null {
    const refactoringIndicators = changes.filter(c =>
      c.hunks.some(h => {
  ...
  private detectDocumentationPattern(changes: GitChange[]): SessionPattern | null {
  ...
  private detectHotfixPattern(session: DevelopmentSession): SessionPattern | null {
    const branch = session.baseBranch.toLowerCase();
  ...
  private detectAntiPatterns(session: DevelopmentSession): DevelopmentAntiPattern[] {
    const antiPatterns: DevelopmentAntiPattern[] = [];
    const allChanges = [...session.workingChanges, ...session.stagedChanges];
    const stats = session.cumulativeStats;
  ...
  private extractCommonPatterns(sessions: DevelopmentSession[]): string[] {
  ...
  private inferDevelopmentStyle(sessions: DevelopmentSession[]): TeamWorkflowInsights['developmentStyle'] {
    const testPatternCount = sessions.filter(s => 
      s.patterns.some(p => p.type === 'rapid-iteration') // TDD-like
    ).length;

    const featurePatternCount = sessions.filter(s =>
      s.patterns.some(p => p.type === 'feature-branch')
    ).length;

    const refactorPatternCount = sessions.filter(s =>
      s.patterns.some(p => p.type === 'refactoring')
    ).length;

    const totalSessions = sessions.length;
    
    if (testPatternCount > totalSessions * 0.6) return 'test-driven';
    if (featurePatternCount > totalSessions * 0.6) return 'feature-first';
    if (refactorPatternCount > totalSessions * 0.4) return 'iterative';
    return 'mixed';
  }

  private calculateQualityMetrics(sessions: DevelopmentSession[]): WorkflowMetrics {
  ...
  private async generateTeamRecommendations(sessions: DevelopmentSession[]): Promise<WorkflowSuggestion[]> {
    const recommendations: WorkflowSuggestion[] = [];
  ...
  private detectTeamAntiPatterns(sessions: DevelopmentSession[]): string[] {
  ...
  private groupFilesByLogicalBoundary(changes: GitChange[]): GitChange[][] {
    const groups: GitChange[][] = [];
    const testFiles = changes.filter(c => this.isTestFile(c.file));
    const docFiles = changes.filter(c => this.isDocFile(c.file));
    const configFiles = changes.filter(c => this.isConfigFile(c.file));
    const codeFiles = changes.filter(c => 
      !this.isTestFile(c.file) && !this.isDocFile(c.file) && !this.isConfigFile(c.file)
    );

    if (codeFiles.length > 0) groups.push(codeFiles);
  ...
  private describeFileGroup(files: GitChange[]): string {
    if (files.every(f => this.isTestFile(f.file))) return 'Test updates';
    if (files.every(f => this.isDocFile(f.file))) return 'Documentation changes';
    if (files.every(f => this.isConfigFile(f.file))) return 'Configuration updates';
    
    const mainFile = files[0];
    if (files.length === 1) return `${mainFile.file} changes`;
    return `${mainFile.file} and ${files.length - 1} related files`;
  }

  private detectCodeDuplication(changes: GitChange[]): string[] {
  ...
  private identifyFileConcerns(changes: GitChange[]): string[] {
    const concerns = new Set<string>();
    
    changes.forEach(change => {
      if (this.isTestFile(change.file)) concerns.add('testing');
      else if (this.isDocFile(change.file)) concerns.add('documentation');
      else if (this.isConfigFile(change.file)) concerns.add('configuration');
      else if (change.file.includes('ui') || change.file.includes('component')) concerns.add('ui');
      else if (change.file.includes('api') || change.file.includes('service')) concerns.add('backend');
      else if (change.file.includes('util') || change.file.includes('helper')) concerns.add('utilities');
      else concerns.add('core');
    });

    return Array.from(concerns);
  }

  private mapAntiPatternToSuggestionType(antiPatternType: DevelopmentAntiPattern['type']): WorkflowSuggestion['type'] {
